...
```json
{
  "format": { "version": "0.3", "generator": "@webarkit/nft-tracker 0.1.0" },
  "extensionsUsed": [],
  "extensionsRequired": [],
  "meta": { },
  "pyramid": { },
...
**Explicit `null` on an optional key.** The rest of §5 only ever says what an *absent* optional key means; it says nothing about a key that is *present* with the value `null`, which readers MUST NOT treat the same as absent by default. An explicit `null` is `BAD_MANIFEST` for **every** optional key — `format.generator`, `extensionsUsed`, `extensionsRequired`, `keypoints.detector.params`, `descriptorSets[].params`, `patches`, `referenceImage` and `info` — because none of them has a defined meaning for "present but null", so a reader has no way to know whether the writer meant "absent" or something else.

Through format `0.2` the two extension arrays were the odd pair out: an explicit `null` on either was accepted and read as an empty array. That was never a decision. It was the behaviour of a `?? []` fallback in the first implementation, which the second matched deliberately rather than diverge silently, and which `0.2 rev 5` then wrote down — so the text documented a probable bug as though it were a rule. Nothing else in §5 gives `null` a meaning, and §7.3 already omits both arrays when they are empty, so no conforming writer has ever emitted one. Format `0.3` removes the exception: all eight optional keys behave the same way.
...
`physicalSizeMm` is **required**: an absent key is `BAD_MANIFEST`, and only an explicit `null` means the physical size is unknown. §7.3's list of optional objects and arrays the canonical writer omits when empty is exactly `params`, `extensionsUsed` and `extensionsRequired` — `physicalSizeMm` is not among them, so a canonical writer always emits it, and requiring it here rejects no file a conforming writer would produce.
...
**The order is the file's, not the reader's.** §7.3 has the canonical writer sort `descriptorSets` by that same key. A reader MUST preserve the order the file gives and MUST NOT sort on decode. Normalising on decode is the coherent alternative and is rejected: it makes the decoded order differ from the file's, which anything indexing sets positionally would silently disagree about, and it buys nothing, since re-sorting on encode already means `encode(decode(f))` lands on the canonical form whatever order `f` used. This constrains implementations, not files: no file is accepted or rejected because of the order its sets arrive in.
...
**Total materialised bytes.** A reader's total materialised accessor bytes MUST NOT exceed the `BIN` chunk's length. Accessors do not overlap (§5.2), so the union of every distinct accessor's byte range is already bounded by that length — but nothing in §5.2 limits how many manifest *fields* may name one accessor, and a reader that materialises once per reference therefore allocates in proportion to the number of references instead. At §6.4's descriptor-set limit, a file well under the file-size limit whose every set names one large `data` accessor decodes to roughly a gigabyte. A reader satisfies this rule by materialising each distinct accessor at most once and handing every field that names it the same array; when fields alias, that is the only way to satisfy it.

This is a bound, not a new limit. It adds no number that §6.4 would have to justify from first principles, because it is derived from §5.2. And it is what completes the opening sentence of this section: "never allocates in proportion to a size before that size has been checked" is a promise about each allocation on its own, and says nothing about their sum.

**Aliasing stays legal.** §5.6 forces `kpIndex` to be the identity in any file without `WKNF_multiview`, so every descriptor set's `kpIndex` array is byte-identical to every other's; at §6.4's defaults, letting sixteen sets name one accessor saves 60 MB of the 64 MB file budget. The canonical writer never aliases — §7.3 gives each array its own accessor — but making a third-party writer's aliased file `BAD_LAYOUT` would cost that deduplication to buy a property the rule above already gives.
...
Fixtures live in a directory shared by `vitest` and `cargo test` (e.g. `fixtures/nft-target/0.3/`). They are produced by a committed, deterministic generator script and never edited by hand:
...
- `invalid/` — at least one file per error code in §6.2, each paired with its expected code, **plus one file per validation rule of §§5.2, 5.4, 5.6, 5.7 and 5.8**: a fractional `offset`; a negative `count`; a `count` above `2^32 − 1`; an accessor reference that is not an integer index below `accessors.length`; a level size of `0`; a level size above `2^16 − 1`; a `scaleStep` of `1`; `levelSizes` growing between two levels; a set with `levelStart[0] ≠ 0`; a set with `levelStart[L] ≠ M`; a `kpIndex` referencing a keypoint of another level; a patch whose `level[q] ≥ L`; a patch rectangle crossing the right or bottom edge of its level; a `referenceImage.level ≥ L`; a `kpIndex` that is a within-level permutation (`M = N`, no repeated value, levels agreeing) without `WKNF_multiview`; a `patchSize` of `0`; an absent `meta.physicalSizeMm`; an explicit `null` on `extensionsUsed`. **Plus one file per I-JSON check of §5**: a manifest with a duplicate member name; one with the same name written twice in different ways (`"a"` and `"\u0061"`), which catches an implementation that compared the raw text instead of the unescaped names; one with an unpaired surrogate escape in a string; one whose `params` carries the integer literal `9007199254740992` (`2^53`); one whose `params` carries `1e400`, which rounds to infinity; one with a **raw** U+FFFF in a `params` value, which strict UTF-8 decoding accepts and only check (e) catches; one with an escaped `"\uFDD0"` in a member name. `NO_USABLE_DESCRIPTORS` is the one exception to "one file per error code": it is produced by the descriptor-set selection of §6.3, against a runtime backend's capabilities, not by decoding, so no file yields it on its own.
- `warnings/` — files that decode successfully with an exact list of expected warnings: unknown chunk, unknown descriptor `kind` next to a valid set, unknown `norm`, unknown `elementType` next to a valid set, unknown optional extension. The `kind` and `elementType` cases are both required because they are what makes §5.6's split observable: the first set survives decoding and re-encodes unchanged, the second is dropped and does not reappear, and only a fixture for each distinguishes an implementation that honours the rule from one that treats every unusable set alike.
- `noncanonical/` — valid files that are **not** what the canonical writer (§7.3) would produce, each paired with the `valid/` file it is equivalent to: different manifest key order; insignificant whitespace; an explicit empty `params`; an unknown top-level key; an unknown non-required extension payload on a descriptor set; **`params` whose keys are unsorted and include `"9"` and `"10"`**, which catches an implementation that serialized them with `JSON.stringify`; **`descriptorSets` in an order other than §7.3's**, which catches both a reader that sorts on decode and a writer that does not sort on encode (§5.6).
...
3. **Non-canonical inputs:** every `noncanonical/` fixture decodes to the same values as its `valid/` counterpart — compared **after ordering `descriptorSets` by §7.3's canonical key**, because §5.6 has the reader preserve the file's order and a `noncanonical/` file may present it in another — and `encode(decode(f))` equals **that counterpart** — not the input. The ordering tolerance applies to the first half only. The byte-identity half is what proves the writer normalises, so it stays strict. This is what makes §7.3's "keeps only what it understands" testable rather than a disclaimer. "Equals" means byte identity for the implementation that wrote the corpus, and item 4's comparison — `BIN\0` byte-identical, manifests equal after parsing — for any other, since §7.3's last paragraph leaves number formatting free across languages (Q8).
...
- A file declaring format `0.4` is rejected by a `0.3` reader with `UNSUPPORTED_FORMAT_VERSION`.
...
- **Backward-compatibility corpus.** Every released format version freezes its fixtures under `fixtures/nft-target/<version>/`. From then on, every reader either decodes them correctly or rejects them with an explicit error — never misreads them.
  ...
  The first frozen corpus is `fixtures/nft-target/0.2/`, released as `v0.2.0`. Every `0.3` implementation MUST reject each of its `valid/` files with `UNSUPPORTED_FORMAT_VERSION` under §7.1's exact-minor rule, and both codecs carry that test. A frozen corpus is not a maintained one: the fixtures this revision adds to `0.3/` have no counterpart in `0.2/`, and that gap stays permanent — harmlessly, because the readers its coverage would have protected no longer exist.
...
- **Q11 — The filter behind a patch's level image.** §5.7 defines patch `q`'s pixels as `level_image[level[q]][…]`, but no text says how a level above 0 was produced from level 0: §5.4 records each level's size and not its downsampling filter. A tracker aligns stored patches against a pyramid of the live frame, and the two are comparable only if both were filtered the same way. `packages/nft-tracker` filters both alike, with one function (`buildFramePyramid`, `packages/nft-tracker/src/tracking/frame_pyramid.ts`): `compile-target` (`packages/nft-tracker/bin/compile-target.mjs`) cuts its patches from the level images that function builds from the target image, and the tracker aligns them on the levels it builds from the live frame. That holds only while one implementation compiles targets and tracks them. Whether the file should record the filter, or the specification should fix one, could not be decided before a first measurement of alignment error across levels. **That measurement now exists** (`packages/nft-tracker/test/tracking/align_patch_accuracy.test.ts`): on rendered views of the pinball test image, with both pyramids built by `buildFramePyramid` and patches of levels 0 to 5, each viewed at its own level's scale so that every one is read on frame level 0, the median position error was between 0.016 and 0.042 px at every level (the test asserts under 0.05 px), while the median photometric gain rose from 0.86 at level 0 to 1.10 at level 5. That is the evidence a decision on this question would be written from; none has been taken, and the question stays open.
...
- **0.2 rev 4** (2026-09-13) — from the second implementation: the `kpIndex` identity becomes a reader rule, not only a writer one, since a permutation has no repeated value and would otherwise decode (§5.6, §8.3); `patchSize` gains the domain `[1, 2^32 − 1]`, with the reason it differs from `dimensions` (§5.7); the `meta`, `referenceImage` and `patchSize` domains are stated with their error code and step, so that an out-of-range size is `BAD_MANIFEST` at step 6 rather than reaching step 7 (§5.3, §5.7, §5.8, §6.1, §6.2); the §5.1 subset rule gets a code and an order against `UNSUPPORTED_EXTENSION` (§6.1, §6.2); §8.2 item 3's byte identity is scoped the way item 4 already is, for an implementation reading a corpus another one wrote (§8.2). Two of these — the `kpIndex` identity and the `patchSize` minimum — narrow reader acceptance rather than being purely editorial: before this revision both a non-identity `kpIndex` (without `WKNF_multiview`) and a `patchSize` of `0` decoded. Nothing is broken by the narrowing, since no released file or codec predates it, but it is not a no-op on the set of files a reader accepts the way the rest of this entry is.
- **0.2 rev 5** (2026-09-13) — editorial, from the whole-branch review of the second implementation: `physicalSizeMm` is stated as required, an absent key being `BAD_MANIFEST` and only an explicit `null` meaning unknown, since §7.3's omit-when-empty list already excludes it (§5.3); what an explicit `null` means for each optional key is stated for the first time — `BAD_MANIFEST` for `format.generator`, `keypoints.detector.params`, `descriptorSets[].params`, `patches`, `referenceImage` and `info`, and an empty array for `extensionsUsed`/`extensionsRequired` (§5.1). Both codecs already enforce these nine rules; this revision only writes them down, so that a third implementation written from the text would not diverge from both with no fixture to catch it. No change to the bytes or the meaning of any valid `0.2` file.
- **0.3** (2026-09-21) — from the issues the second implementation's review left open. **Normative, and narrowing:** an explicit `null` on `extensionsUsed` or `extensionsRequired` is now `BAD_MANIFEST` like the other six optional keys (§5.1), so a file a shipped `0.2` reader accepts no longer decodes. That narrowing is why this is a new minor rather than a sixth revision of `0.2`. `0.2 rev 4` made two comparable narrowings inside a revision and justified it in as many words — "no released file or codec predates it" — and that justification expired when `v0.2.0` tagged two codecs advertising format `0.2` and froze `fixtures/nft-target/0.2/` under §8.3. Two readers both claiming `0.2` while disagreeing about a file is exactly what §7.1's exact-minor rule exists to prevent. **Normative, but changing no file's validity:** a reader preserves the file's `descriptorSets` order and MUST NOT sort on decode (§5.6); a reader's total materialised accessor bytes MUST NOT exceed the `BIN` chunk length (§6.1), a bound §5.2 already implied and no text had stated, which aliased accessor references could otherwise blow past by a factor of the descriptor-set limit. **Editorial:** §8.2 item 3's "same values" is scoped to allow for set order, its byte-identity half left strict; §8.1 gains fixtures for four rules that had none and for the unsorted-sets case; §8.3 names the frozen `0.2` corpus and what a `0.3` reader owes it; Q8 records that the cross-implementation tolerance turned out to be needed by every fixture, not held in reserve (§11); the header stops calling both implementations unbuilt.
- **0.3 rev 1** (2026-09-24) — editorial: open question Q11 (§11), the downsampling filter behind a patch's level image, raised while defining the tracker's patch-alignment types. No change to the bytes or the meaning of any valid `0.3` file.
- **0.3 rev 2** (2026-09-26) — editorial: open question Q11 (§11) records that the first measurement of alignment error across levels, which it was waiting for, now exists, with what it found, and that the question stays open; its account of the one filtering function now covers `compile-target` as well as the tracker, and it names the file that implements `buildFramePyramid` in place of the one that declares its type. No change to the bytes or the meaning of any valid `0.3` file.
