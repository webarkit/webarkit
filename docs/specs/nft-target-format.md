...
**Explicit `null` on an optional key.** The rest of §5 only ever says what an *absent* optional key means; it says nothing about a key that is *present* with the value `null`, which readers MUST NOT treat the same as absent by default. An explicit `null` is `BAD_MANIFEST` for `format.generator`, `keypoints.detector.params`, `descriptorSets[].params`, `patches`, `referenceImage` and `info` — none of these has a defined meaning for "present but null", so a reader has no way to know whether the writer meant "absent" or something else. The two extension arrays are the odd pair out: an explicit `null` for `extensionsUsed` or `extensionsRequired` is accepted and treated as an empty array, matching how §7.3 already omits them when empty.
