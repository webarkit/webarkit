...
- **Q11 — The filter behind a patch's level image.** §5.7 defines patch `q`'s pixels as `level_image[level[q]][…]`, but no text says how a level above 0 was produced from level 0: §5.4 records each level's size and not its downsampling filter. A tracker aligns stored patches against a pyramid of the live frame, and the two are comparable only if both were filtered the same way. The TypeScript tracker currently keeps them comparable by building both pyramids with one function (`buildFramePyramid`, `packages/nft-tracker/src/tracking/types.ts`). That holds only while one implementation compiles targets and tracks them. Whether the file should record the filter, or the specification should fix one, is open until the first measurement of alignment error across levels.
