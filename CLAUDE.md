...
- **Never round-trip a source file through PowerShell** (`Get-Content -Raw` then `Set-Content`). It reads as the system ANSI codepage and writes UTF-8, so every non-ASCII character is double-encoded (`§` becomes `Â§`) and a BOM is prepended. Source here is full of `§` section references, so the damage is silent and wide. Edit files with the Edit/Write tools or a POSIX tool; use PowerShell to *run* things, not to rewrite them.
