...
export class JsfeatNextBackend implements CvBackend {
    ...
    detect(image: GrayImage, options?: DetectOptions): Keypoint[] {
        const img = toMatrix(image);
        jsfeatNext.fast_corners.set_threshold(options?.threshold ?? 20);

        const count = this.detectInto(img, BORDER);

        // Strongest first, so an explicit cap keeps the best corners rather
        // than whichever ones the raster scan reached first.
        const found = this.pool.slice(0, count);
        ...
        // Orientation is not optional: `orb.describe` rotates each patch by the
        // angle it is handed, so leaving it unset would rectify every patch by
        // the keypoint_t default of -1 radian. Filling it here means `detect`
        // returns keypoints that are immediately usable by `describe`, which is
        // what the contract's `Keypoint.angle` promises.
        return found.map((k) => ({
            x: k.x,
            y: k.y,
            score: k.score,
    ...
    describe(image: GrayImage, keypoints: Keypoint[], options?: DescribeOptions): Descriptors {
        ...
        const descriptors = new jsfeatNext.matrix_t(ORB_BYTES, Math.max(count, 1), U8C1);

        if (count > 0) {
            if (this.pool.length < count) this.pool = makePool(count);
            ...
            jsfeatNext.orb.describe(img, this.pool, count, descriptors);
        }

        return {
            // Copied: `descriptors` is local here, but the contract requires
            // caller ownership unconditionally and a future pooled buffer must
            // not quietly change that guarantee.
            data: new Uint8Array(descriptors.data.subarray(0, count * ORB_BYTES)),
            count,
            bytesPerDescriptor: ORB_BYTES,
            kind: "orb",
            norm: "hamming",
        };
    }
